...
const HORIZON_RE = /^\s*horizon:\s*(now|backlog)\s*$/i
...
function parsePlan(text) {
  ...
  for (const raw of text.split('\n')) {
    ...
    if ((m = line.match(TASK_RE))) {
      ...
      lastNode = { id: m[3], title: m[2], level: 'task', parent: curComponent ? curComponent.id : null, needs: [], links: [], tech: '', by: '', horizon: '', status }
      nodes.push(lastNode)
      continue
    }
    if ((m = line.match(TECH_RE))) { if (lastNode) lastNode.tech = m[1]; continue }
    if ((m = line.match(BY_RE))) { if (lastNode) lastNode.by = m[1]; continue }
    if ((m = line.match(HORIZON_RE))) { if (lastNode) lastNode.horizon = m[1].toLowerCase(); continue }
