// OpenRouter summarizer: one short line per session saying what it is doing now.
//
...
const MAX_TOKENS = 80;
const TEXT_MAX = 180;       // hard cut on the reply, however the model reads the word limit
const MATERIAL_MAX = 4000;  // what one summary is allowed to cost, in chars
const COOLDOWN_MS = 300_000;
const RETENTION = 7 * 86_400_000;

// Two caps, always: the prompt asks for 25 words and the code cuts at 180 chars.
// A model that ignores the instruction still cannot push a paragraph into a card.
const SYSTEM = 'Summarize what this coding session is doing right now. '
  + 'Same language as the conversation. At most 25 words. No preamble.';
...
export function createSummarizer({
  ...
  const cachePath = path.join(stateDir, 'summaries.json');
  const logPath = path.join(stateDir, 'summaries-log.jsonl');
  try { fs.mkdirSync(stateDir, { recursive: true }); } catch {}

  const cache = new Map();       // sessionId -> {text, at, version}
  const nextAt = new Map();      // sessionId -> earliest ms for the next attempt
  const inflight = new Set();    // sessionIds with a refresh in the air
  ...
  // costs nothing a lazy read would have saved.
  const log = [];
  let dropped = false;
  for (const line of (() => { try { return fs.readFileSync(logPath, 'utf8').split('\n'); } catch { return []; } })()) {
    if (!line) continue;
  ...
  async function refresh(id, material) {
    try {
      ...
      const text = redact(String(data?.choices?.[0]?.message?.content ?? '')).trim().slice(0, TEXT_MAX);
  ...
  return {
    ...
    get(sessionId, material) {
      ...
      if (due) {
        // Claimed before the await so a burst of ticks produces one request.
        inflight.add(sessionId);
        nextAt.set(sessionId, Date.now() + minIntervalMs);
        refresh(sessionId, material);
      }
