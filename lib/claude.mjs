...
const SPAWN_HEAD = 8 << 10;          // bytes read from the head of an agent transcript
const SPAWN_CHARS = 4000;            // spawn prompt kept per agent (the summarizer's budget)
const SPAWN_AGENTS = 80;             // newest agents a session reads a spawn prompt for
...
// ---- spawn prompt of a subagent --------------------------------------------
/**
 * The prompt an agent was spawned with, read from the HEAD of its own
 * transcript: that prompt IS the first user record, so a session with thousands
 * of agents never pays more than SPAWN_HEAD bytes for one, and the scan stops at
 * the first user record rather than walking the file. A prompt whose record does
 * not fit the window is unparseable here and yields null — a label is not worth
 * an unbounded second read.
 */
function headPrompt(p) {
  let buf, fd;
  try {
    fd = fs.openSync(p, 'r');
    buf = Buffer.allocUnsafe(SPAWN_HEAD);
    buf = buf.subarray(0, fs.readSync(fd, buf, 0, SPAWN_HEAD, 0));
  } catch { return null; } finally { if (fd !== undefined) try { fs.closeSync(fd); } catch {} }
  ...
  for (const line of buf.toString('utf8').split('\n')) {
    ...
    const text = typeof c === 'string' ? c
      : Array.isArray(c) ? (c.find((b) => typeof b?.text === 'string')?.text ?? '')
      : '';
    return text.trim().slice(0, SPAWN_CHARS) || null;   // the first user record, or nothing
  }
  return null;
}
...
export function createClaudeAdapter(opts = {}) {
  ...
  function scanAgents(s) {
    ...
    // A spawn prompt is only read for an agent that can actually be rendered:
    // everything running, plus the newest SPAWN_AGENTS of the tree. A real
    // session reaches thousands of agents and the rest are never drawn, so the
    // tick must never walk their transcripts.
    const shown = agents.length > SPAWN_AGENTS
      ? new Set([...agents].sort((x, y) => (y.startedAt || 0) - (x.startedAt || 0)).slice(0, SPAWN_AGENTS))
      : null;
    for (const a of agents) {
      a.spawnPrompt = !shown || shown.has(a) || a.status === 'running' ? agentPrompt(c, a) : null;
    }

    workflows.forEach((w, i) => {
      w.runningAgents = pendingLive[i].map((a) => ({
        agentId: a.agentId, description: a.description, currentTool: a.currentTool,
        spawnPrompt: a.spawnPrompt ?? null,
      }));
    });
  ...
  /**
   * Cached per agent for the daemon's lifetime: a spawn prompt is immutable, so
   * the head of an agent transcript is read exactly once, however many passes
   * the agent survives. A file that is not there yet — or is still empty in the
   * window between creation and its first line — settles on a later pass instead
   * of caching a permanent null.
   */
  function agentPrompt(c, a) {
    if (!a.transcriptPath) return null;
    const cache = (c.prompts ||= new Map());
    if (cache.has(a.agentId)) return cache.get(a.agentId);
    const prompt = headPrompt(a.transcriptPath);
    if (prompt === null && !a.size) return null;
    cache.set(a.agentId, prompt);
    return prompt;
  }
  ...
  return {
    ...
    /**
     * The spawn prompt of ONE named agent, on demand. The emit path only reads
     * the newest SPAWN_AGENTS of a session, and a click can name any agent in a
     * tree of thousands — so this one has no recency cap, just the same bounded
     * read and the same per-agent cache. An unknown session or agent is null,
     * never a throw.
     */
    spawnPrompt(sessionId, agentId) {
      const s = S.get(sessionId);
      const a = s?.tree?.agents.find((x) => x.agentId === agentId);
      if (!a) return null;
      return a.spawnPrompt ?? agentPrompt(s.agentCache, a);
    },
